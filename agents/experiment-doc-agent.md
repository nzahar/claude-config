...
**Size cap (unconditional, before the no-drift short-circuit).** For each in-scope `REPORT.md`, run the size-cap check per [`lib/doc-compaction-contract.md`](../lib/doc-compaction-contract.md): count the structural-portion lines (total minus the results/metrics blocks) and, if over the hard cap, compact. Emit the soft-cap WARNING when in the warning band; for an un-delimited legacy REPORT.md (no clear `## Result`), key on total lines and run the delimit-first pass per the contract before compacting. REPORT.md has **no delete-eligible duplicate section** (no `Module exports` / sorted-path-list analog), so size pressure resolves only by trimming over-budget table cells per the contract's cell rule — cut content that merely restates the numbers, move genuine caveats verbatim into the `## Caveats / open questions` prose (incrementally, not in bulk). The quoted metrics under `## Result` (and `## Metrics`, if present) up to the next `## ` are the protected no-touch zone — verbatim from the notebook per the Hard rules, never reworded, trimmed, or relocated, and excluded from the structural-line count. This runs unconditionally — before the no-drift short-circuit (step 6 below).
...
- **D8 (doc size hard-cap): applies.** REPORT.md is subject to the size-triggered compaction in [`lib/doc-compaction-contract.md`](../lib/doc-compaction-contract.md) (Phase 1 size step above). Per-artifact specifics: the protected no-touch block is the results/metrics; there is no delete-eligible duplicate section, so compaction trims over-budget cells (cut prose that merely restates the numbers, move genuine caveats verbatim per the contract's cell rule) and never deletes a REPORT.md section.
