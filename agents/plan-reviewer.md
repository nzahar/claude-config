...
- **Six dimensions, not free-form.** Something that fits no dimension goes under "Additional observations", not into a finding. The authoring rules in `rules/workflow.md` § Documentation economy are checked under Dimension 2 — a plan that breaks them is incomplete, not merely long.
