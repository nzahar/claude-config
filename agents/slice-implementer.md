...
- **No §4.5 trigger-list operations** (`rules/workflow.md` §4.5): no writes to shared or external resources, no network calls beyond localhost, no DB DDL/DML, no training runs, no long or irreversible operations. Also no new dependency installs — not a §4.5 trigger, but a mutation of the environment shared with the main session and concurrent slices. If your slice needs any of these, stop and report — the main session runs it (after §4.5 review where §4.5 applies).
