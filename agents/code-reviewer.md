...
Apply `rules/workflow.md` § Documentation economy as MEDIUM findings; an unresolved Decision in a plan or several unrelated decisions in one ADR is HIGH. README, `skills/*` and prose outside the doc tree are out of scope. Pre-existing issues go under `## Tech debt (pre-existing)`.
