...
Apply detection procedures D1–D8 from `rules/workflow.md` § Documentation economy. Map findings to this agent's native severity vocabulary (`CRITICAL`/`HIGH`/`MEDIUM`/`LOW`/`NEEDS VERIFICATION`) — **do not import `plan-reviewer`'s `blocker`/`warning` vocabulary**.
...
- **D1, D2, D4, D6, D7, D8 → MEDIUM.** Smell-level — flag and continue. Scope per workflow.md for each rule (D8: `docs/plans/*.md` only).
- `CRITICAL`, `LOW` and `NEEDS VERIFICATION` do not apply to doc-economy findings: nothing here is runtime/security critical, detection is mechanical (no ambiguity to defer), and no rule maps below MEDIUM.
