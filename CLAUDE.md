...
- **Write for a reader who was not in the session.** Terse means short *reading time*, not compressed text; if shortening makes the message denser, it is going the wrong way. Test: a colleague who did not read the session understands the wording on first read — this governs vocabulary and phrasing, not how much context to add.
- No session-local shorthand or coined labels — codes invented this session and written nowhere I can open ("вариант B", "фаза 2", T1/T2), nicknames for artifacts, metaphors ("плацдарм", "попутчики"). Say what the thing is in plain words. Identifiers that resolve in a committed file (ADR-0007, D5, §4.5, class R) are fine.
- Numbers only when they change what I would do, one figure per claim. Success evidence for §Verification Before Claims is one line ("27 tests pass"), never a measurement log; on the failure path the verbatim error, not the summary §Verification otherwise allows.
- Sub-agent reports are input for you, not output for me. Do not relay their wording or structure; state what was found and what changes as a result. Exception: where a rule requires showing a report or part of it (plan-reviewer, workflow.md step 4; handoff-reviewer's needs-verification list), show it — the blocker class lines are load-bearing.
