...
- No session-local shorthand or coined labels — codes invented this session and written nowhere I can open (S4, T1, Q7), nicknames for artifacts, metaphors ("плацдарм", "попутчики"). Say what the thing is in plain words. Identifiers that resolve in a committed file (ADR-0007, D5, §4.5, class R) are fine.
- Numbers only when they change what I would do, one figure per claim. Success evidence for §Verification Before Claims is one line ("27 tests pass"), never a measurement log; a failure carries the verbatim error, as §Verification requires.
- Sub-agent reports are input for you, not output for me. Do not relay their wording or structure; state what was found and what changes as a result. Exception: where a rule requires showing a report (plan-reviewer, workflow.md step 4), show it — the blocker class lines are load-bearing.
