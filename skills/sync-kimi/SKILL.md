---
name: sync-kimi
description: "Regenerate Kimi Code's user profile (~/.kimi-code/AGENTS.md, agents/, lib and skills symlinks) from the Claude framework in ~/.claude by running scripts/sync-kimi.sh. Invoke when the user types /sync-kimi or asks to update / sync Kimi with the framework (обнови кими, синкни кими, подтяни фреймворк в кими). Works the same from Claude Code and from Kimi Code. Never edits framework files — generation only."
argument-hint: "[--check]"
---
